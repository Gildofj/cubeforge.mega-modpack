...
{
	const static int MAX_QUESTS = static_cast<int>(QuestType::QuestType_END);
	if (MAX_QUESTS == 0)
	{
		return QuestType(0);
	}
...
int cube::Quest::GetSubType()
{
	...
	return subtypes[(this->modifier / static_cast<int>(QuestType::QuestType_END)) % cnt];
}
...
void cube::Quest::Complete()
{
	...
	if (game)
	{
		game->PrintMessage(L"Completed quest!\n", 64, 255, 64);
		if (game->GetPlayer())
		{
			game->GetPlayer()->gold += this->GetReward().amount;
		}
	}
}
...
{
	this->num_spirits = static_cast<char>((std::min)(progress, static_cast<int>(CHAR_MAX)));
}
