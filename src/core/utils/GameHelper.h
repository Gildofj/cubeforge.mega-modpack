#pragma once

#include "cwsdk.h"
#include "../memory/MemoryHelper.h"

namespace cube::Helper
{
	enum class ItemGenerationType
	{
		Artifact = 0,
		Pet,
		Gold,
		Gear,
		Consumable,
	};

	int CWGetItemsSold(cube::World* world, cube::Item item, long long seller_id);
	void AnnounceItem(cube::Game* game, cube::Item* item, unsigned int count, cube::Creature* creature);
	void LevelUp(cube::Creature* player);
	cube::Item GenerateItem(ItemGenerationType type, IntVector2 region = IntVector2(0, 0));
	void DropItem(cube::Creature* creature, const cube::Item& item, int amount = 1);
	void DropItem(cube::Creature* creature, ItemGenerationType type, int amount = 1);

	int CheckIfCreatureExists(cube::Game* game, __int64 id);
	__int64 UpdateCreatureID(cube::Game* game, __int64 id);

	inline bool InGUI(cube::Game* game)
	{
		if (!game) return false;
		cube::GUI* gui = &game->gui;
		plasma::Node* nodes[] = {
			gui->startmenu_node,
			gui->character_selection_node,
			gui->character_creation_node,
			gui->world_selection_node,
			gui->world_creation_node,
			gui->equipment_inventory_widget ? gui->equipment_inventory_widget->node : nullptr,
			gui->crafting_inventory_widget ? gui->crafting_inventory_widget->node : nullptr,
			gui->multiplayer_widget ? gui->multiplayer_widget->node : nullptr,
			gui->vendor_inventory_widget ? gui->vendor_inventory_widget->node : nullptr,
			gui->adaption_widget ? gui->adaption_widget->node : nullptr,
			gui->voxel_widget ? gui->voxel_widget->node : nullptr,
			gui->enchant_widget ? gui->enchant_widget->node : nullptr,
			gui->question_widget ? gui->question_widget->node : nullptr,
			gui->system_widget ? gui->system_widget->node : nullptr,
			gui->controls_widget ? gui->controls_widget->node : nullptr,
			gui->options_widget ? gui->options_widget->node : nullptr
		};

		for (plasma::Node* node : nodes) {
			if (!node || !node->display) continue;
			auto data = &node->display->visibility.data;
			auto frame = node->display->visibility.current_frame;

			if (frame < data->size() && data->at(frame)) {
				return true;
			}
		}

		if (gui->chat_widget && gui->chat_widget->typebox_active) return true;
		if (gui->map_open) return true;
		if (gui->esc_menu_displayed) return true;

		return false;
	}

	void LoreInteraction(cube::Creature* player, int percentage);
	bool PositionContainsWater(const LongVector3& position);

	inline static int RandomInt()
	{
		static long long count = 0;
		int random = rand();
		count += random;
		if (count >= (long long)1000 * INT_MAX)
		{
			count -= (long long)1000 * INT_MAX;
		}
		std::srand(static_cast<unsigned int>(random + count));
		return random;
	}
	
	inline static float RandomZeroToOne()
	{
		return (float)(RandomInt() / (float)(RAND_MAX));
	}

	inline void DisableCreatureFloating()
	{
		MemoryHelper::WriteByte((void*)CWOffset(0x2BF67A), 0xEB);
	}

	inline void EnableCreatureFloating()
	{
		MemoryHelper::WriteByte((void*)CWOffset(0x2BF67A), 0xFF);
	}
}
